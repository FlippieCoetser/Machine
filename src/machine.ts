...
export class Machine<Attributes extends {}, Events extends {}> extends Emitter {
  private _default;
  private _state;
  private _cache;
  ...
  private _targetAttribute = (event: Events) => this._target(event)?.attribute;
  ...
  private _getEvents = (attribute) => ({
    attribute: attribute,
    event:
      this.configuration.states[attribute].states[this.state[attribute]].on,
  });
  ...
  private _invokeActions = (actions, state, ...args) =>
    actions.forEach((action) =>
      this.configuration.actions[action](this, state, ...args)
    );
  ...
  get events() {
    return this.attributes.map(this._getEvents);
  }
  get defaults(): any[] {
